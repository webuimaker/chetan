...
```{r fig.cap="Distribution of sample means",out.width="50%"}
require(ggplot2)
...
```{r fig.cap="Distribution of sample means (proper histogram)", out.width="50%"}
...
```{r fig.cap="Bootstrap sampling distribution of sample mean",out.width="50%"}
plot(B)
```

Or, one can use `ggplot` to get the same figure

```{r fig.cap="Bootstrap sampling distribution of sample mean  (ggplot2)",out.width="50%" }
...
```{r fig.cap="Distribution of observed sample birthweights",out.width="50%"}
...
```{r fig.cap="Parametric bootstrap using boot package",fig.align="center",out.width="50%"}
plot(B)
```
